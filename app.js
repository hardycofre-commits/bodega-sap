...
function setSaveInfo(text, cls='ok'){
  const el = $('saveInfo'); el.className = 'pill ' + cls; el.textContent = text;
}
function setSapInfo(text, cls=''){
  const el = $('sapInfo'); el.className = 'pill ' + cls; el.textContent = text;
}
function saveLocal(){ localStorage.setItem(STORAGE_KEY, JSON.stringify(sheetStore)); }
function loadLocal(){
  try { sheetStore = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}') || {}; }
  catch { sheetStore = {}; }
}
...
async function cargarUltimoExcelGithub(){
  try{
    setSapInfo('Buscando último Excel en GitHub...');
    ...
    setSapInfo('Último SAP cargado: ' + file.name, 'ok');
  }catch(e){
    setSapInfo('Sin Excel automático. Usa carga manual o sube un Excel a datos.', 'warn');
  }
}
...
function cargarSheetsJSONP(){
  return new Promise(resolve => {
    ...
    const timeout = setTimeout(() => {
      cleanup(); setSaveInfo('Sin conexión a Google Sheets. Usando respaldo local.', 'warn'); resolve(false);
    }, 12000);
    ...
    window[cb] = rows => {
      try{
        ...
        (rows || []).forEach(r => {
          ...
          if(!code) return;
          next[code] = {
            real: r.stock_real !== '' && r.stock_real !== undefined ? Number(r.stock_real) : undefined,
        ...
        sheetStore = next;
        saveLocal();
        setSaveInfo('Sincronizado con Google Sheets', 'ok');
        renderAll();
        cleanup(); resolve(true);
      }catch(e){
        setSaveInfo('Error leyendo Google Sheets. Usando respaldo local.', 'error');
        cleanup(); resolve(false);
      }
    };
    const script = document.createElement('script');
    script.src = SHEETS_WEBAPP_URL + '?callback=' + cb + '&t=' + Date.now();
    script.onerror = () => { cleanup(); setSaveInfo('Sin conexión a Google Sheets. Usando respaldo local.', 'warn'); resolve(false); };
    document.body.appendChild(script);
  });
}

async function sincronizarRegistro(codigo){
  ...
  saveLocal();
  setSaveInfo('Guardando en Google Sheets...', 'warn');
  try{
    ...
    setSaveInfo('Sin conexión. Cambio guardado localmente.', 'warn');
  }
}
...
function estadoTexto(e,m){
  if(e === 'ok') return '🟢 Coincide';
  if(e === 'rebajar') return `🔴 Rebajar ${m.sap - Number(rec(m.codigo).real || 0)}`;
  if(e === 'revisar') return '🟡 Revisar diferencia';
  if(e === 'cero') return '🔵 SAP 0 con stock';
...
function renderKpis(){
  ...
  for(const m of materiales){
    ...
    if(e === 'pendiente' && !rec(m.codigo).oculto && m.sap > 0) p++;
    if(e === 'ok') ok++;
    if(e === 'rebajar') reb++;
    if(e === 'revisar') rev++;
    if(e === 'cero') cero++;
  }
  $('kTotal').textContent=t; $('kPend').textContent=p; $('kOk').textContent=ok;
  $('kReb').textContent=reb; $('kRev').textContent=rev; $('kCero').textContent=cero;
}
function baseFiltrada(){
  const q = $('searchInput').value.toLowerCase().trim();
  let base = materiales.filter(m => q ? m.search.includes(q) : true);
  if(filtroActual === 'pendiente') base = base.filter(m => estado(m)==='pendiente' && m.sap > 0 && !rec(m.codigo).oculto);
  else base = base.filter(m => estado(m) === filtroActual);
  return base;
}
function renderResults(){
  const q = $('searchInput').value.toLowerCase().trim();
  let base = q ? materiales.filter(m => m.search.includes(q)) : baseFiltrada();
  if(!q && filtroActual !== 'pendiente') base = baseFiltrada();
  if(!q && filtroActual === 'pendiente') base = baseFiltrada();
  const total = base.length;
  const list = base.slice(0,250);
  $('results').innerHTML = `<p class="small"><b>${total}</b> materiales encontrados${total>250?' · mostrando primeros 250':''}</p>` +
    list.map(m => `<div class="item" data-code="${m.codigo}"><div class="code">${m.codigo} · SAP ${m.sap} ${m.um||''}</div><div class="desc">${m.desc}</div><div class="meta">${estadoTexto(estado(m),m)} · Última revisión: ${rec(m.codigo).fecha || rec(m.codigo).fechaOculto || 'sin registro'}</div></div>`).join('');
}
function abrir(codigo){
  ...
  $('dCode').textContent=current.codigo; $('dDesc').textContent=current.desc;
  $('dSap').textContent=current.sap + ' ' + (current.um || '');
  const r = rec(current.codigo);
  $('realInput').value = r.real ?? '';
  $('dFecha').textContent = 'Última revisión: ' + (r.fecha || r.fechaOculto || 'sin registro');
...
function volverBuscador(){
  $('detailPanel').classList.add('hidden'); $('homePanel').classList.remove('hidden');
  current = null; renderAll(); setTimeout(() => $('searchInput').focus(), 50);
}
...
async function guardarRevision(){
  ...
  await sincronizarRegistro(current.codigo);
  $('searchInput').value = '';
  volverBuscador();
}
async function ocultarMaterial(){
  if(!current) return;
  sheetStore[current.codigo] = {...rec(current.codigo), oculto:true, fechaOculto:new Date().toLocaleDateString('es-CL')};
  await sincronizarRegistro(current.codigo);
  $('searchInput').value = '';
  volverBuscador();
}
...
$('hideBtn').addEventListener('click', ocultarMaterial);
$('realInput').addEventListener('input', actualizaEstadoDetalle);
document.querySelectorAll('.chip').forEach(btn => btn.addEventListener('click', () => {
  document.querySelectorAll('.chip').forEach(b => b.classList.remove('active'));
  btn.classList.add('active'); filtroActual = btn.dataset.filter; renderAll();
}));
$('fileInput').addEventListener('change', async e => {
  const f = e.target.files[0]; if(!f) return;
  ultimoArchivo = f.name;
  setSapInfo('Archivo manual cargado: ' + f.name, 'ok');
  const ab = await f.arrayBuffer(); procesarWorkbook(XLSX.read(ab, {type:'array'}));
});

async function init(){
  loadLocal();
  renderAll();
  await Promise.all([cargarUltimoExcelGithub(), cargarSheetsJSONP()]);
  renderAll();
}
init();
